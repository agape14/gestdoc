...
export default function Create({ folderId = null, breadcrumbLabel = '' }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('plantillas-ing.store'), { forceFormData: true });
    };

    const cancelUrl = folderId ? route('plantillas-ing.index', { folder_id: folderId }) : route('plantillas-ing.index');

    return (
        <MainLayout>
            <Head title="Nueva Plantilla de Ingeniería" />
            <div className="card border-0 shadow-sm p-4 rounded-4 bg-body form-card-responsive" style={{ maxWidth: '800px', margin: '0 auto' }}>
                <div className="mb-4">
                    <h3 className="fw-bold mb-1">Nueva Plantilla de Ingeniería</h3>
                </div>
