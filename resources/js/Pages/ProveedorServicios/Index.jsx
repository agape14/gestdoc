...
export default function Index({ servicios, groupedByEspecialidad, filters, flash, userRole }) {
    const { auth } = usePage().props;
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (search !== (filters.search || '')) {
                router.get(route('proveedor-servicios.index'), { ...filters, search }, {
                    preserveState: true,
                    preserveScroll: true,
                    replace: true,
                });
    ...
    return (
        <MainLayout>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                <div>
                    <h2 className="fw-bold text-body mb-0">Proveedor de Servicios {filters.tipo ? `(${filters.tipo}s)` : ''}</h2>
                    <p className="text-secondary mb-0">Gestión de servicios públicos y privados</p>
                </div>
                <div className="d-flex gap-2 flex-wrap">
                    <Link href={route('proveedor-servicios.index', { tipo: 'Publica' })} className={`btn ${filters.tipo === 'Publica' ? 'btn-primary' : 'btn-outline-primary'} rounded-pill px-4`}>
                        <i className="bi bi-building me-2"></i> PÚBLICAS
                    </Link>
                    <Link href={route('proveedor-servicios.index', { tipo: 'Privada' })} className={`btn ${filters.tipo === 'Privada' ? 'btn-primary' : 'btn-outline-primary'} rounded-pill px-4`}>
                    ...
                    {currentUserRole !== 'Visualizador' && (
                        <>
                            ...
                            <Link href={route('proveedor-servicios.create')} className="btn btn-success shadow-sm rounded-pill px-4">
